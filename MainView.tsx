...
const MainView = ({ isAdmin, members, setMembers, announcement, setAnnouncement, systemStatus, setSystemStatus, currentTrack, setTrack, userProfile }: MainViewProps) => {
  ...
  const handleUpdateVibe = () => {
    if (!vibeData.url.startsWith('http')) {
      alert("Tolong pakai link yang bener ya (diawali http:// atau https://)");
      return;
    }
    setTrack(vibeData);
    ...
    <div className="overflow-x-hidden">
        ...
        <div className="container mx-auto max-w-7xl">
          <div className="glass p-4 md:p-6 rounded-[32px] border-accent/20 bg-accent/10 pointer-events-auto flex flex-col md:flex-row items-center gap-4 md:gap-8 animate-in slide-in-from-top-10 duration-700">
            <div className="flex items-center gap-3">
              ...
              <span className={`text-[10px] font-tech ${getStatusColor()} uppercase tracking-widest font-bold`}>KABAR TERBARU:</span>
            </div>
            {isEditingAnnounce ? (
              <div className="flex-1 flex flex-wrap md:flex-nowrap gap-3 w-full">
                ...
                <select className="bg-black/40 border border-white/10 rounded-xl px-3 text-white text-[10px] font-tech py-2" value={systemStatus} onChange={(e) => setSystemStatus(e.target.value)}>
                  <option value="Optimal">Aman Terkendali</option>
                  <option value="Maintenance">Sedang Perbaikan</option>
                  <option value="Emergency">Darurat!</option>
                </select>
                <button onClick={() => setIsEditingAnnounce(false)} className="px-6 py-2 bg-accent text-white text-[10px] font-tech rounded-xl">SIMPAN</button>
              </div>
            ) : (
      ...
      <section className="min-h-screen flex flex-col justify-center pt-52 pb-16 px-5 md:px-10 relative">
        <div className="container mx-auto max-w-7xl">
          <h1 className="hero-text font-serif italic text-white mb-8">
            Wajah Baru <br />
            <span className="not-italic text-transparent bg-clip-text bg-gradient-to-r from-white via-slate-200 to-slate-500">Keluarga Besar</span> <br />
            X TJKT 2
          </h1>
          <div className="flex flex-col sm:flex-row items-center gap-6">
            <button onClick={() => document.getElementById('squad')?.scrollIntoView({ behavior: 'smooth' })} className="px-10 py-5 bg-white text-black font-tech font-bold text-[11px] tracking-widest rounded-full hover:bg-accent hover:text-white transition-all uppercase flex items-center gap-3">Kenalan Yuk <ArrowRight size={16}/></button>
            <div className="flex items-center gap-3 text-slate-600 font-tech text-[9px] uppercase tracking-widest bg-white/5 px-5 py-2 rounded-full border border-white/5">
              <Activity size={14} className={`${getStatusColor()} animate-pulse`} /> Kondisi Kelas: {systemStatus === 'Optimal' ? 'Aman' : systemStatus}
            </div>
          </div>
        </div>
      </section>
          ...
          <div className="container mx-auto max-w-7xl">
            <div className="glass p-10 md:p-16 rounded-[50px] border-accent/10 flex flex-col md:flex-row items-center gap-12">
              <div className="flex-1">
                <p className="text-accent font-tech text-[10px] tracking-[0.4em] uppercase mb-4">Ruang Musik</p>
                <h2 className="text-4xl md:text-5xl font-serif italic text-white mb-6">Atur Suasana Kelas</h2>
                <p className="text-slate-500 font-tech text-xs tracking-widest uppercase mb-8">Ganti suasana kelas dengan pasang lagu favorit di website ini secara otomatis.</p>
                ...
                {isEditingVibe ? (
                  <div className="space-y-4 max-w-md animate-in fade-in zoom-in duration-300">
                    ...
                    <div className="relative">
                      ...
                      <input className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 pl-12 pr-6 text-white text-xs font-tech" placeholder="Link MP3 Langsung..." value={vibeData.url} onChange={e => setVibeData({...vibeData, url: e.target.value})}/>
                    </div>
                    ...
                    <div className="p-4 bg-accent/5 border border-accent/10 rounded-2xl flex gap-3 items-start">
                      ...
                      <p className="text-[10px] font-tech text-slate-400 leading-relaxed uppercase tracking-wider">
                        Saran: Gunakan link yang ujungnya <span className="text-white">.mp3</span>. Bisa pakai dari <span className="text-accent underline">catbox.moe</span> atau Discord.
                      </p>
                    </div>

                    <div className="flex gap-3">
                      <button onClick={handleUpdateVibe} className="flex-1 py-4 bg-accent text-white font-tech font-bold text-[10px] rounded-xl shadow-lg shadow-accent/20">PUTAR MUSIK</button>
                      <button onClick={() => setIsEditingVibe(false)} className="px-6 py-4 bg-white/5 text-slate-500 font-tech text-[10px] rounded-xl hover:text-white transition-colors">BATAL</button>
                    </div>
                  </div>
                ) : (
                  <button onClick={() => setIsEditingVibe(true)} className="px-10 py-5 border border-accent/30 text-accent font-tech font-bold text-[11px] tracking-widest rounded-2xl hover:bg-accent hover:text-white transition-all">GANTI LAGU</button>
                )}
              </div>
              <div className="w-full md:w-80 aspect-square glass rounded-[40px] flex items-center justify-center relative overflow-hidden group">
                 ...
                 <div className="text-center p-8">
                    ...
                    <span className="text-[10px] font-tech text-slate-500 uppercase block mb-1">Sedang Diputar</span>
      ...
      <section id="squad" className="py-24 md:py-32 px-5 md:px-10 bg-[#030303]">
        <div className="container mx-auto max-w-7xl text-center mb-16">
          <h2 className="text-6xl md:text-8xl font-serif italic text-white mb-6">Teman Sekelas</h2>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {members.slice(0, showAllMembers ? members.length : 8).map((m, i) => (
            <div key={i} className={`glass p-8 rounded-[32px] border-l-[3px] transition-all hover:bg-white/[0.05] ${m.priority ? 'border-accent' : 'border-transparent'}`}>
              ...
              <p className="text-[9px] font-tech uppercase tracking-widest text-slate-500">{m.role === 'Wali Kelas' ? 'Ibu Wali Kelas' : m.role}</p>
            </div>
          ))}
        </div>
        {!showAllMembers && members.length > 8 && (
          <div className="text-center mt-12">
            <button onClick={() => setShowAllMembers(true)} className="text-accent font-tech text-[10px] tracking-widest uppercase hover:underline">Lihat Teman Lainnya</button>
          </div>
        )}
      </section>
    </div>
